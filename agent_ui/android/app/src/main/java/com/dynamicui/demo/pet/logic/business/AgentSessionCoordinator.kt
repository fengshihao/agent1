...
class AgentSessionCoordinator(
    ...
    private val emitUiEvent: (AgentUiEvent) -> Unit,
    ...
    fun initRuntimeIfPossible() {
        ...
        if (key.isEmpty()) {
            emitUiEvent(AgentUiEvent.Error("未配置 DASHSCOPE_API_KEY"))
            return
        }
        val voicePrompt = try {
            appContext.assets.open(VOICE_PROMPT_ASSET).bufferedReader().use { it.readText() }.trim()
        } catch (e: Exception) {
            emitUiEvent(AgentUiEvent.Error("无法读取语音助手提示词: " + (e.message ?: "")))
            return
        }
        if (voicePrompt.isBlank()) {
            emitUiEvent(AgentUiEvent.Error("语音助手提示词为空"))
            return
        }
    ...
    override fun onEvent(event: AgentEvent) {
        ...
        when (event.type) {
            AgentEventType.AGENT_START -> {
                ...
                emitUiEvent(AgentUiEvent.BusyChanged(true))
                emitUiEvent(AgentUiEvent.Streaming(""))
            }
            AgentEventType.MESSAGE_UPDATE -> {
                val p = event.payload as? EventPayloads.MessageUpdate ?: return
                emitUiEvent(AgentUiEvent.Streaming(p.partialMessage.content))
            }
            AgentEventType.AGENT_END -> {
                ...
                emitUiEvent(AgentUiEvent.BusyChanged(false))
                when {
                    abortRequested.getAndSet(false) -> {
                        hadErrorThisRun = false
                        emitUiEvent(AgentUiEvent.Aborted(finalText))
                    }
                    hadErrorThisRun -> {
                        hadErrorThisRun = false
                    }
                    else -> emitUiEvent(AgentUiEvent.Finished(finalText))
                }
            }
            AgentEventType.AGENT_ERROR -> {
                ...
                emitUiEvent(AgentUiEvent.BusyChanged(false))
                emitUiEvent(AgentUiEvent.Error(p?.message ?: "未知错误"))
            }
    ...
    fun submitUserMessage(text: String): Boolean {
        val rt = runtime ?: run {
            emitUiEvent(AgentUiEvent.Error("Agent 未初始化，请检查 API Key"))
            return false
        }
        val msg = text.trim()
        ...
        return try {
            ...
            emitUiEvent(AgentUiEvent.Error("上一轮尚未结束，请稍候"))
            false
        } catch (e: Exception) {
            emitUiEvent(AgentUiEvent.Error(e.message ?: "请求失败"))
            false
        }
    }
