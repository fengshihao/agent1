...
class JavaBackedLlmUiAgent(
    private val runtimeAdapter: JavaAgentRuntimeAdapter,
    ...
    suspend fun generateUiJson(intent: String): Result<String> {
        ...
        """.trimIndent()
        return runtimeAdapter.chat(prompts.generationSystemPrompt, userPrompt).map(::cleanModelJson)
    }
    ...
    suspend fun summarizeSelection(
        ...
        """.trimIndent()
        return runtimeAdapter.chat(prompts.summarySystemPrompt, userPrompt)
    }
}
