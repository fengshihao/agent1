#!/bin/sh
set -eu
# 子工程内实现：uv 一键安装全局命令 agent1。
# 仓库根请用 install-python-agent.sh / install-python-agent.ps1（薄转调，便于 curl 固定路径）。

AGENT1_GIT_URL="${AGENT1_GIT_URL:-git+https://github.com/fengshihao/agent1.git#subdirectory=python_agent}"
