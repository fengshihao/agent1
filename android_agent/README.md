...
Gradle 工程位于 `android_agent/` 根目录（与 `app/`、`gradlew` 同级）；**下文「后续增量」已在当前代码中实现**，详细运行方式、API Key 与 Tab 说明以 `QUICKSTART.md` 为准。
...
以下能力在后续提交中加入，**GitHub 请以 `main` 为准**；若仓库默认分支仍为 `master`，需将 `master` 与 `main` 同步（见文末「仓库分支」），否则网页上看到的 `android_agent` 可能是旧快照。
